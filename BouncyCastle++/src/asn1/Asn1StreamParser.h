...
#ifndef _ASN1STREAMPARSER_H
#define	_ASN1STREAMPARSER_H
...
namespace org {
  namespace bouncycastle {
    namespace asn1 {
      ...
      public:
        Asn1StreamParser(Stream inStream) : this(inStream, int.MaxValue) {
        }

        Asn1StreamParser(Stream	inStream, int limit) {
          if (!inStream.CanRead) {
        ...
        Asn1StreamParser(byte[] encoding) : this(new MemoryStream(encoding, false), encoding.Length) {
        }
      ...
      private:
        ...
        Asn1EncodableVector loadVector(byte[] bytes) {
